...
def signup_wrong_password_test(driver: Chrome, credentials: dict):
    ...
    confirm_password.send_keys(credentials['wrong_password'])
...
def signup_short_password_test(driver: Chrome, credentials: dict):
    target_site = 'http://localhost:5000/signup'
    driver.get(target_site)
    ...
    password = driver.find_element_by_id('password')
    password.clear()
    password.send_keys(credentials['short_password'])

    confirm_password = driver.find_element_by_id('conf_password')
    confirm_password.clear()
    confirm_password.send_keys(credentials['short_password'])
...
def signup_wrong_email_test(driver: Chrome, credentials: dict):
    target_site = 'http://localhost:5000/signup'
    driver.get(target_site)
    ...
    confirm_email = driver.find_element_by_id('conf_email')
    confirm_email.clear()
    confirm_email.send_keys(credentials['wrong_email'])
...
def signup_right_test(driver: Chrome, credentials: dict):
    target_site = 'http://localhost:5000/signup'
    driver.get(target_site)
