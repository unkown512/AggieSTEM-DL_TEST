  ...
  if(request.method == 'POST'):
    if(form.validate_on_submit()):
      ...
      if(user_manager.validate_user(db, email, pw)):
        ...
        new_user = User(user_profile, form.password.data, str(user_id), user_access_level)
...
def manage_users():
  ...
  if(request.method == 'GET'):
    ...
    return render_template('manage_users.html', user=current_user.username, data=data,
      access_level=int(current_user.access))
  else:
    #POST
    db = db_client()
...
import os
app.secret_key = "asdfasfds"
app.run(host='138.68.45.210', port='5000')
